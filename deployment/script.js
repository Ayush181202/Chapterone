...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCountdown() {
        const eventDate = new Date('April 22, 2025 19:00:00 GMT').getTime();
        ...
        document.title = `Chapter One - ${days}d ${hours}h ${minutes}m ${seconds}s until April 22nd event!`;
    }
    ...
    // Fix image paths with spaces when published
    document.querySelectorAll('img').forEach(img => {
        const src = img.getAttribute('src');
        if (src) {
            // Replace spaces with %20 for all URLs to ensure proper loading
            if (src.includes(' ')) {
                img.src = src.replace(/ /g, '%20');
                console.log('Fixed space in image path:', img.src);
            }
            
            // Specific handling for key images
            if (src.includes('Core Bar.png') || 
                src.includes('CoreBar.png') ||
                src.includes('Step 1.png') || 
                src.includes('Step 2.png') || 
                src.includes('Step 3.png')) {
                
                // Store the original source for debugging
                const originalSrc = img.src;
                
                // Ensure URL encoding is properly applied
                img.src = img.src.replace(/ /g, '%20');
                
                console.log(`Special image handling: ${originalSrc} -> ${img.src}`);
                
                // Add special loading handling for event card image
                if (src.includes('Core Bar.png') || src.includes('CoreBar.png')) {
                    // Add loading indicator
                    const parent = img.parentElement;
                    if (parent && parent.classList.contains('event-image')) {
                        ...
                        // Handle load event
                        img.onload = function() {
                            console.log('Core Bar image loaded successfully');
                            if (loadingIndicator) {
                        ...
                        // Handle error event with multiple fallback attempts
                        img.onerror = function() {
                            console.error('Failed to load Core Bar image:', this.src);
                            
                            // Try with various path formats
                            const fallbackPaths = [
                                '/images/CoreBar.png',
                                '/images/Core%20Bar.png',
                                'images/CoreBar.png',
                                'images/Core%20Bar.png',
                                'images/Core Bar.png'
                            ];
                            ...
                            const nextPathIndex = fallbackPaths.findIndex(path => path === currentPath) + 1;
                            
                            if (nextPathIndex > 0 && nextPathIndex < fallbackPaths.length) {
                                console.log(`Trying fallback path: ${fallbackPaths[nextPathIndex]}`);
                                this.src = fallbackPaths[nextPathIndex];
                            } else {
                                // If all paths failed, show error message
                                console.error('All image paths failed to load');
                                if (loadingIndicator) {
                                    loadingIndicator.innerHTML = '<div class="error-message">Image could not be loaded</div>';
                                }
                            }
                        };
                    }
...
function createSVGFolder() {
    ...
    document.querySelectorAll('.step-image img').forEach(img => {
        ...
        const altKey = alt.toLowerCase().includes('step 1') ? 'questionnaire' :
                      alt.toLowerCase().includes('step 2') ? 'algorithm' :
                      alt.toLowerCase().includes('step 3') ? 'match' : '';
        
        if (svgs[altKey]) {
            const svgContainer = document.createElement('div');
            svgContainer.innerHTML = svgs[altKey];
